...
export const ERR_REMOTE_OP_NOT_FOUND = 'Remote `op` not found'
export const ERR_SELECT_OPTION_INVALID = 'Select option is required and must contain at least one op name in the manifest'
...
export const ERR_KIND_NOT_RECOGNIZED = (kind) => `No templates found for Op kind ${kind}, please select a different kind`
export const ERR_TEMPLATE_NOT_FOUND = (from) => `Unable to find template ${from}`
export const ERR_TEMPLATE_DOWNLOAD_FAILED = ({ from, err }) => `Unable to fetch template ${from}. (${err.message})`
export const ERR_DOCKER_CONTAINER_FAILURE = (err) => `Error creating Docker container ${err.message}`
export const ERR_REGISTRY_PULL_FAILED = (err) => `Error pulling registry image ${err.message}`
